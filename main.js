...
let paidToday = document.querySelector(".progress .inner .number span");
...
if(localStorage.getItem("paidTodayDate")) {

    if(localStorage.getItem("paidTodayDate") != paidTodayChecker.getDate()) {
        localStorage.setItem("paidTodayDate", paidTodayChecker.getDate());
        localStorage.setItem("paid-today", 100);
        paidToday.innerHTML = 100;
        editCircle();
    }

} else {
    localStorage.setItem("paidTodayDate", 0);
}
...
let A_descriptionInput = document.querySelector(".balance-add-form .description");
let R_descriptionInput = document.querySelector(".balance-remove-form .description");
    ...
    if(addForm.style.display != "none") {
        A_doneBtn.addEventListener("click", () => {
            if(A_amountInput.value != "" && A_descriptionInput.value != "") {
                ...
                let newBalance = parseInt(localStorage.getItem("balance")) + parseInt(A_amountInput.value);
                ...
                let newBox = {
                    ...
                    date: `${activityDate.getDate()}/${activityDate.getMonth() + 1}/${activityDate.getFullYear()} ${activityDate.getHours()}:${activityDate.getMinutes()}:${activityDate.getSeconds()}`,
                    caret: "in"
                }
    ...
    if(removeForm.style.display != "none") {
        R_doneBtn.addEventListener("click", () => {
            if(R_amountInput.value != "" && R_descriptionInput.value != "") {
                ...
                let newBalance = parseInt(localStorage.getItem("balance")) - parseInt(R_amountInput.value);
                localStorage.setItem("balance", newBalance);
                updateBalance();

                let newPaidToday = parseInt(paidToday.innerHTML) - parseInt(R_amountInput.value);
...
function editCircle() {
    if(localStorage.getItem("paid-today") < 0) {
        ...
        circle.style["stroke-dashoffset"] = 472 - 472 * localStorage.getItem("paid-today") / 100 + 15;
    }
}
